...
from entmoot.optimizer.optimizer import Optimizer as opt_entmoot

from training_Sch_NN import main as training
from DFO_ext_all import wrapper, wrapper_bi, wrapper_tri
...
def large(config):
    ...
    Nt=5
    data[None].update({'N_t': {None: Nt}, 'Tc': {None: np.arange(1, 1+Nt)}})
    res = centralised_all(data, size='hard', regression_type = 'NN', nodes=(n1,n2), time_limit=7200)
    t = time.perf_counter()
    print(f"Objective: {pyo.value(res.obj)} in {(t - t0)/60} min")
...
class Function(BenchmarkFunction):
    def __init__(self):
        self.name = '2_d small opt'
    
    def get_bounds(self, n_dim: int=2):
        return [Integer(5, 80), Integer(1, 40)]

    def _eval_point(self, X):
        config = {'nodes1': X[0], 'nodes2': X[1]}
        return tuple(small(config)['objectives'])
