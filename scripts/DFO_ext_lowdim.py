...
from hierarchy.planning.Planning_Scheduling_Control import scheduling_Asia_bi # , centralised
...
def get_cost_bi(input_data, Production, TP, Sales, penalization=1):
    ...
    return (prod_cost + transp_cost + store_cost + storage + changeover - sales)/1e6 + penalty*penalization
...
def get_cost_tri(input_data, Production, TP, Sales, penalization=1):
    ...
    return (prod_cost + transp_cost + store_cost + changeover + storage + energy - sales)/1e6 + penalty*penalization
