...
from training_Sch_hierarch import main as training
...
from hierarchy.planning.Planning_Integr_Real import centralised_all
...
def small(config):
    ...
    try:
        dir = f"./results/Models/hierarch_RegNN_{n1}_{n2}.json"
        with open(dir, 'r+') as file:
            file_data = json.load(file)
    except:
        parser = parse_args()
    ...
    return {'objectives': [file_data['opt_time_s'], file_data['hierarchy']['real']['obj']]}
...
def medium(config):
    ...
    return {'objectives': [file_data['opt_time_m'], file_data['medium']['real']['obj']]}
...
def large(config):
    ...
    Nt=5
    data[None].update({'N_t': {None: Nt}, 'Tc': {None: np.arange(1, 1+Nt)}})
    res = centralised_all(data, size='large', regression_type = 'NN', nodes=(n1,n2), time_limit=1800)
    t = time.perf_counter()
    print(f"Objective: {pyo.value(res.obj)} in {(t - t0)/60} min")
    ...
    return {'objectives': [file_data['opt_time_l'], file_data['large']['real']['obj']]}
...
def main():
    ...
    history.plot_pareto_front()
    plt.show()
    plt.savefig('results/Figures/pareto_hierarch_large1.svg')

    history.plot_hypervolumes(logy=True)
    plt.show()
    plt.savefig('results/Figures/hypervolume_hierarch_large1.svg')

    history = opt.get_history()
    try:
        print(history)
        text_file = open("results/Optima/surrogates_hierarch_large_opt.txt", "w")
    ...
    history.plot_pareto_front()
    plt.show()
    plt.savefig('results/Figures/pareto_hierarch_large2.svg')

    history.plot_hypervolumes(logy=True)
    plt.show()
    plt.savefig('results/Figures/hypervolume_hierarch_large2.svg')
