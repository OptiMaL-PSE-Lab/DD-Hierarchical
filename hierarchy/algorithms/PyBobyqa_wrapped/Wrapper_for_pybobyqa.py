...
from hierarchy.algorithms.general_utility_functions import PenaltyFunctions
...
class PyBobyqaWrapper:
    ...
    def create_dictionary_for_solution(self, sol):
        ...
        dummy = [output_dict['x_store'][0]] ; min_ = output_dict['f_store'][0]
        for i, value in enumerate(self.Penaly_fun.f_his):
            if value < min_:
                dummy += [self.Penaly_fun.x_his[i]]
                min_ = value
            else:
                dummy += [dummy[-1]]
        output_dict['x_best_so_far'] = dummy
