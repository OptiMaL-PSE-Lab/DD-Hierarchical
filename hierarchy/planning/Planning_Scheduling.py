...
# from data.planning_sch import data as import_data
...
def Asia(data, arg="Augmented"):
    ...
    model.CCH = pyo.Param(model.P, model.P)
    model.Y = pyo.Var(model.P, model.T, within=pyo.Binary)
    model.B = pyo.Var(model.P, model.P, model.T, within=pyo.Binary)
    ...
    def CoAs(m):
        ...
        changeover = sum(
            m.CCH[p1, p2]*m.B[p1, p2, t] for p2 in m.P for p1 in m.P for t in m.T if p1 != p2
            ) + sum(
                1000*m.Y[p, t] for p in m.P for t in m.T
            )
        return m.Cost_As == prod_cost + store_cost + transp_cost + changeover - sales
    ...
    def link_B(m, p, t):
        return sum(m.B[p, p2, t] for p2 in m.P) <= 1
    def link_B1(m, p1, p2, t):
        if p1 != p2:
            return m.B[p1, p2, t] + m.B[p2, p1, t] <= m.Y[p1, t]
        else:
            return m.B[p1, p2, t] + m.B[p2, p1, t] == 0
    def link_B2(m, p1, p2, t):
        if p1 != p2:
            return m.B[p1, p2, t] + m.B[p2, p1, t] <= m.Y[p2, t]
        else:
            return pyo.Constraint.Skip

    def link_Prod_Y(m, p, t):
        return m.Prod[p, "Asia", t] <= m.Y[p, t]*1e7

    def link_B_Y2(m, t):
        lhs = 0.5*sum(m.B[p1, p2, t] + m.B[p2, p1, t] for p2 in m.P for p1 in m.P)
        rhs = sum(m.Y[p, t] for p in m.P) - 1
        return lhs >= rhs

    def link_B_Y1(m, p, t):
        lhs = sum(m.B[p, p2, t] + m.B[p2, p, t] for p2 in m.P)
        rhs = m.Y[p, t] 
        return lhs >= rhs

    model.Blink = pyo.Constraint(model.P, model.T, rule=link_B)
    model.B1link = pyo.Constraint(model.P, model.P, model.T, rule=link_B1)
    model.B2link = pyo.Constraint(model.P, model.P, model.T, rule=link_B2)
    model.PYlink = pyo.Constraint(model.P, model.T, rule=link_Prod_Y)
    model.BY2link = pyo.Constraint(model.T, rule=link_B_Y2)
    model.BY1link = pyo.Constraint(model.P, model.T, rule=link_B_Y1)
...
def America(data, arg="Augmented"):
    ...
    def link_Prod_Y(m, p, t):
        return m.Prod[p, "America", t] <= m.Y[p, t]*1e7
