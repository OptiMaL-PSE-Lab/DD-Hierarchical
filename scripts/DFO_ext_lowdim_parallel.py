...
def parallel_tri(P_set, X, data_, Production, t):
    ...
    # print('Average input to scheduling problem: ', scheduling_data[None]['Prod'])
    try:
        res_Sch = scheduling_Asia_bi(data_)
        changeover = pyo.value(res_Sch.CCH_cost)
        storage = pyo.value(res_Sch.st_cost)/20
        energy = pyo.value(sum(res_Sch.energy_1[n] + res_Sch.energy_2[n] for n in res_Sch.N))/12000
    except:
        changeover = 100./12*1e6
        storage = 10/12*1e6
        energy = 10/12*1e6
...
with open('./results/optima/tri_Py-BOBYQA_distr.json', 'w') as f:
    json.dump(tri_opt, f)
